...
router.get('/list', function(req,res){
    ...
    async.waterfall([function(callback){
      TopBanner.find(function(err, docs){
        res.json(docs);
      }).sort({"_id" : -1});
    }]);
  });
...
router.get('/', function(req,res){
  ...
  async.waterfall([function(callback){
      BannerCounter.findOne({name:"topbanner"}, function (err,counter) {
        if(err) callback(err);
      ...
      TopBanner.count(search.findPost,function(err,count){
        if(err) callback(err);
        skip = (page-1)*limit;
      ...
      TopBanner.find(search.findPost).populate("author").sort('-createdAt').skip(skip).limit(limit).exec(function (err,topbanner) {
        if(err) callback(err);
        callback(null, topbanner, maxPage);
      });
    }],function(err, topbanner, maxPage){
      if(err) return res.json({success:false, message:err});
      return res.render("topbanner/index",{
        topbanner:topbanner, user:req.user, page:page, maxPage:maxPage,
...
router.get('/new', isLoggedIn, function(req,res){
  res.render("topbanner/new", {user:req.user});
});    // new
  ...
  async.waterfall([function(callback){
    BannerCounter.findOne({name:"topbanner"}, function (err,counter) {
      ...
      if(counter){
        ...
        BannerCounter.create({name:"topbanner",totalCount:0},function(err,counter){
          if(err) return res.json({success:false, message:err});
          callback(null, counter);
        });
      }
    ...
    TopBanner.create(req.body.post,function (err,post) {
      if(err) return res.json({success:false, message:err});
      counter.totalCount++;
      counter.save();
      res.redirect('/topbanner');
    });
  });
});  // create
...
router.get('/:id', function(req,res){
  ...
  .exec(function (err,post) {
    ...
    res.render("topbanner/show", {post:post, url:url, urlQuery:req._parsedUrl.query,
...
router.get('/:id/edit', isLoggedIn, function(req,res){
  TopBanner.findById(req.params.id, function (err,post) {
    // var url = req.protocol + '://' + req.get('host') + '/images/' + post._id;
    var url = 'https://plinic.s3.ap-northeast-2.amazonaws.com/' + post.filename;
    ...
    res.render("topbanner/edit", {post:post, prefilename:prefilename, preoriginalName:preoriginalName, url:url, user:req.user});
  });
});  // edit
...
router.put('/:id', s3upload.single('image'), isLoggedIn, function(req, res, next){
  try{
    ...
    s3.deleteObject(s3parmas, function(err, data){
      ...
      TopBanner.findOneAndUpdate({_id:req.params.id, author:req.user._id}, req.body.post, function (err,post) {
        if(err) return res.json({success:false, message:err});
        if(!post) return res.json({success:false, message:"No data found to update"});
        res.redirect('/topbanner/'+req.params.id);
      });
    });
  }catch(err){
...
router.delete('/:id', isLoggedIn, function(req,res, next){
  try{
    TopBanner.findOneAndRemove({_id:req.params.id, author:req.user._id}, function (err,post) {
...
router.post('/:id/comments', function(req,res){
  ...
  TopBanner.update({_id:req.params.id},{$push:{comments:newComment}},function(err,post){
    if(err) return res.json({success:false, message:err});
    res.redirect('/topbanner/'+req.params.id+"?"+req._parsedUrl.query);
  });
}); //create a comment


router.delete('/:postId/comments/:commentId', function(req,res){
  TopBanner.update({_id:req.params.postId},{$pull:{comments:{_id:req.params.commentId}}},
    function(err,post){
      if(err) return res.json({success:false, message:err});
      res.redirect('/topbanner/'+req.params.postId+"?"+
