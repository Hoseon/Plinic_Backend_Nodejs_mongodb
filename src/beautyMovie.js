...
router.get('/like/:id/:email', function(req, res) {
  async.waterfall([function(callback) {
    BeautyMovie.findOne({ posturlId : req.params.id },function(err, docs) {
      if(err) {
        return res.json({
          success: false,
          message: err
...
router.get('/oneList/:id', function(req, res) {
  async.waterfall([function(callback) {
    BeautyMovie.findOne({ posturlId : req.params.id },function(err, docs) {
      if(err) {
        return res.json({
...
router.get('/list/:id', function(req, res) {
  console.log("aaa")
  async.waterfall([function(callback) {
    BeautyMovie.findOne({ _id : req.params.id },function(err, docs) {
      if(err) {
        return res.json({
          success: false,
          message: err
  ...
  BeautyMovie.findById(req.params.id)
    .exec(function(err, post) {
      ...
      if (k == 0) {
        post.like++;
        post.save();
        BeautyMovie.update({_id : req.params.id},
...
router.get('/', function(req, res) {
  ...
  async.waterfall([function(callback) {
    ...
    BeautyMovie.find(search.findPost).populate("author").sort('-createdAt').skip(skip).limit(limit).exec(function(err, beautyMovie) {
      if (err) callback(err);
    ...
    return res.render("beautyMovie/index", {
...
router.get('/:id', function(req, res) {
  BeautyMovie.findById(req.params.id)
    ...
    .exec(function(err, post) {
      ...
      // var url = req.protocol + '://' + req.get('host') + '/beautyMovie_images/' + post._id;
      ...
      // var prod_url = req.protocol + '://' + req.get('host') + '/beautyMovie_prodimages/' + post._id;
      //fs.createReadStream(path.join(__dirname, '../uploads/', post.filename)).pipe(res);
      res.render("beautyMovie/show", {
        post: post,
        url: url,
        // prod_url: prod_url,
        urlQuery: req._parsedUrl.query,
  ...
  s3.deleteObject(s3parmas, function(err, data){
    ...
    BeautyMovie.findOneAndUpdate({
      ...
      if (!post) return res.json({
        ...
        res.redirect('/beautyMovie/' + req.params.id);
    });
  });
}); //update
...
router.post('/:id/comments', function(req, res) {
  ...
  BeautyMovie.update({
    ...
    res.redirect('/beautyMovie/' + req.params.id + "?" + req._parsedUrl.query);
  });
}); //create a comment
...
router.delete('/:postId/comments/:commentId', function(req, res) {
  BeautyMovie.update({
    ...
    function(err, post) {
      ...
      res.redirect('/beautyMovie/' + req.params.postId + "?" +
