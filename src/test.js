...
var TestCounter = require('./models/TestCounter');
...
router.get('/', function(req, res) {
  ...
  async.waterfall([function(callback) {
    ...
    Test.count(search.findPost, function(err, count) {
      if (err) callback(err);
      skip = (page - 1) * limit;
    ...
    Test.find(search.findPost).populate("author").sort('-createdAt').skip(skip).limit(limit).exec(function(err, test) {
      if (err) callback(err);
      callback(null, test, maxPage);
    });
  }], function(err, test, maxPage) {
...
router.post('/', s3upload.fields([{
  ...
  async.waterfall([function(callback) {
    ...
    Test.create(req.body.post, function(err, post) {
      if (err) return res.json({
        success: false,
        message: err
      });
...
router.post('/:id/comments', function(req, res) {
  ...
  Test.update({
    ...
    res.redirect('/test/' + req.params.id + "?" + req._parsedUrl.query);
  });
}); //create a comment
...
router.delete('/:postId/comments/:commentId', function(req, res) {
  Test.update({
    ...
    function(err, post) {
      ...
      res.redirect('/test/' + req.params.postId + "?" +
