...
router.get('/', function(req,res){
  ...
  async.waterfall([function(callback){
      ProductCounter.findOne({name:"product"}, function (err,counter) {
        if(err) callback(err);
      ...
      Product.find(search.findPost).populate("author").sort('-createdAt').skip(skip).limit(limit).exec(function (err,product) {
        if(err) callback(err);
      ...
      return res.render("product/index",{
        product:product, user:req.user, page:page, maxPage:maxPage,
...
router.get('/new', isLoggedIn, function(req,res){
  res.render("product/new", {user:req.user});
});    // new
  ...
  async.waterfall([function(callback){
    ProductCounter.findOne({name:"product"}, function (err,counter) {
      ...
      if(counter){
        ...
        ProductCounter.create({name:"product",totalCount:0},function(err,counter){
          if(err) return res.json({success:false, message:err});
          callback(null, counter);
        });
      }
...
router.get('/:id', function(req,res){
  ...
  .exec(function (err,post) {
    ...
    res.render("product/show", {post:post, url:url, urlQuery:req._parsedUrl.query,
...
router.get('/:id/edit', isLoggedIn, function(req,res){
  Product.findById(req.params.id, function (err,post) {
    ...
    res.render("product/edit", {post:post, prefilename:prefilename, preoriginalName:preoriginalName, url:url, user:req.user});
  });
});  // edit
...
router.put('/:id', s3upload.single('image'), isLoggedIn, function(req, res, next){
  try{
    ...
    s3.deleteObject(s3parmas, function(err, data){
      ...
      Product.findOneAndUpdate({_id:req.params.id, author:req.user._id}, req.body.post, function (err,post) {
        if(err) return res.json({success:false, message:err});
...
router.delete('/:id', isLoggedIn, function(req,res, next){
  try{
    Product.findOneAndRemove({_id:req.params.id, author:req.user._id}, function (err,post) {
...
router.post('/:id/comments', function(req,res){
  ...
  Product.update({_id:req.params.id},{$push:{comments:newComment}},function(err,post){
    if(err) return res.json({success:false, message:err});
    res.redirect('/product/'+req.params.id+"?"+req._parsedUrl.query);
  });
}); //create a comment


router.delete('/:postId/comments/:commentId', function(req,res){
  Product.update({_id:req.params.postId},{$pull:{comments:{_id:req.params.commentId}}},
    function(err,post){
      if(err) return res.json({success:false, message:err});
      res.redirect('/product/'+req.params.postId+"?"+
