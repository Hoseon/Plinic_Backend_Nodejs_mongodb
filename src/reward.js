...
var RewardCounter = require('./models/RewardCounter');
...
router.get('/', function(req, res) {
  ...
  async.waterfall([function(callback) {
    ...
    Reward.find(search.findPost).populate("author").sort('-createdAt').skip(skip).limit(limit).exec(function(err, reward) {
      if (err) callback(err);
    ...
    return res.render("reward/index", {
      reward: reward,
      user: req.user,
      page: page,
      maxPage: maxPage,
...
router.post('/', upload.fields([{
  ...
  async.waterfall([function(callback) {
    ...
    Reward.create(req.body.post, function(err, post) {
      if (err) return res.json({
        success: false,
        message: err
      });
...
router.get('/:id', function(req, res) {
  Reward.findById(req.params.id)
    ...
    .exec(function(err, post) {
      ...
      // var url = req.protocol + '://' + req.get('host') + '/reward/' + post._id;
      var url = 'https://plinic.s3.ap-northeast-2.amazonaws.com/' + post.filename;
      // var prod_url = req.protocol + '://' + req.get('host') + '/reward_prodimages/' + post._id;
...
router.get('/:id/edit', isLoggedIn, function(req, res) {
  Reward.findById(req.params.id, function(err, post) {
...
router.delete('/:postId/comments/:commentId', function(req, res) {
  Reward.update({
    ...
    function(err, post) {
      ...
      res.redirect('/reward/' + req.params.postId + "?" +
