...
router.get('/', function(req, res) {
  ...
  async.waterfall([function(callback) {
    ...
    Faq.find(search.findPost).populate("author").sort('-createdAt').skip(skip).limit(limit).exec(function(err, faq) {
      if (err) callback(err);
...
router.get('/:id', function(req, res) {
  Faq.findById(req.params.id)
    ...
    .exec(function(err, post) {
      ...
      // var url = req.protocol + '://' + req.get('host') + '/faq/' + post._id;
      // var url = 'https://plinic.s3.ap-northeast-2.amazonaws.com/' + post.filename;
      // var prod_url = req.protocol + '://' + req.get('host') + '/faq_prodimages/' + post._id;
...
router.post('/:id/comments', function(req, res) {
  ...
  Faq.update({
    ...
    res.redirect('/faq/' + req.params.id + "?" + req._parsedUrl.query);
  });
}); //create a comment
...
router.delete('/:postId/comments/:commentId', function(req, res) {
  Faq.update({
    ...
    function(err, post) {
      ...
      res.redirect('/faq/' + req.params.postId + "?" +
